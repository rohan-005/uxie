...
export const CardTemplateThree: React.FC<CardProps> = ({
  ...
  return (
    <div
      ...
      className={`relative w-full max-w-[440px] aspect-[1/1.52] rounded-2xl bg-black border-2 border-primary-container/80 shadow-[0_24px_60px_-10px_rgba(0,0,0,1),0_0_45px_rgba(222,115,156,0.4)] p-6 flex flex-col justify-between overflow-hidden group select-none ${className}`}
    >
      {/* High Contrast Obsidian Lines & Metallic Accents */}
      ...
      {/* Top Header: Dramatic Vertical Identity & Metallic Badge */}
      <div className="relative z-10 flex items-start justify-between border-b border-outline-variant/40 pb-4">
        ...
        <div className="flex flex-col items-end gap-1.5">
          <div className={`px-3 py-1 rounded border font-code text-label-code-sm font-bold tracking-wider ${rarityConfig.badgeBg} ${rarityConfig.badgeText} ${rarityConfig.borderColor}`}>
            {profile.rarity}
          </div>
      ...
      <div className="relative z-10 my-4 flex items-center justify-between gap-4 flex-grow">
        {/* Left Side: Massive Power Number */}
        ...
        <div className="relative w-36 h-36 sm:w-40 sm:h-40 rounded-xl overflow-hidden border-2 border-primary-container shadow-[0_0_30px_rgba(222,115,156,0.35)] bg-surface-container-lowest p-1">
          <img
            src={displayAvatar}
            alt={profile.displayName}
            className="w-full h-full object-cover rounded-lg filter contrast-110 saturate-110"
            onError={(e) => {
